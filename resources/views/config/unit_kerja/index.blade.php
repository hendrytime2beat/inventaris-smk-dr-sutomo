...
<div class="modal fade" id="modal-main" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <form method="post" action="{{ route('config.unit_kerja.act') }}" class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Unit Kerja Form</h5>
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          @enderror
          <input type="text" name="nama_unit_kerja" id="nama_unit_kerja" placeholder="Nama Unit Kerja" class="form-control" value="{{ old('nama_unit_kerja') }}" required>
        </div>
      </div>
...
<script>
    $('#table-data').DataTable({
      ...
      "bServerSide": true,
      "ajax": {
        "url": "{{ route('config.unit_kerja.list') }}",
        "type": "POST",
        "data": {
            "_token": '{{ csrf_token() }}',
        }
    ...
    function hapus(id) {
      swal({
        ...
        if (result.value) {
          $.get('{{ url("config/unit_kerja/delete") }}/' + id, function(res) {
            swal(
              'Sukses!',
