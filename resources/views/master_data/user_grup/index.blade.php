            ...
            <table class="table align-items-center mb-0" id="table-data">
              <thead>
                <tr>
                  ...
                  <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Nama Grup</th>
...
<div class="modal fade" id="modal-main" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <form method="post" action="{{ route('master_data.user_grup.act') }}" class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">User Grup Form</h5>
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          @enderror
          <input type="text" name="nama_grup" id="nama_grup" placeholder="Nama Grup" class="form-control" value="{{ old('nama_grup') }}" required>
        </div>
      </div>
...
<script>
    $('#table-data').DataTable({
      ...
      "bServerSide": true,
      "ajax": {
        "url": "{{ route('master_data.user_grup.list') }}",
        "type": "POST",
        "data": {
            "_token": '{{ csrf_token() }}',
        }
    ...
    function hapus(id) {
      swal({
        ...
        if (result.value) {
          $.get('{{ url("master_data/user_grup/delete") }}/' + id, function(res) {
            swal(
              'Sukses!',
