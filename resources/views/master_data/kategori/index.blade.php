...
<div class="modal fade" id="modal-main" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <form method="post" action="{{ route('master_data.kategori.act') }}" class="modal-content">
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          @enderror
          <input type="text" name="nama_kategori" id="nama_kategori" placeholder="Nama Kategori" class="form-control" value="{{ old('nama_kategori') }}" required>
        </div>
      </div>
...
<script>
    $('#table-data').DataTable({
      ...
      "bServerSide": true,
      "ajax": {
        "url": "{{ route('master_data.kategori.list') }}",
        "type": "POST",
        "data": {
            "_token": '{{ csrf_token() }}',
        }
    ...
    function hapus(id) {
      swal({
        ...
        if (result.value) {
          $.get('{{ url("master_data/kategori/delete") }}/' + id, function(res) {
            swal(
              'Sukses!',
